...
class Predicate:
  ...
  def evaluate_ranges(self, X):
    ...
    for i in sat_min.size:
      sat_Xranges.append([sat_min[i], sat_max[i]])
      unsat_Xranges.append([unsat_min[i], unsat_max[i]])
    return mask, sat_Xranges, unsat_Xranges


class Line:
  'Linear classifier'
  def __init__(self, lc, feature_filter, cnames, sample_no):
    assert(len(cnames) == 2)
    self.lc = lc
